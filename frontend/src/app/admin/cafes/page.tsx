...
const INITIAL: CafeItem[] = [
  { id: "c1", name: "Brew & Bean", slug: "brew-and-bean", description: "Artisan coffee & homemade pastries", admin: "Cafe Owner", dishes: 14, active: true },
...
export default function CafesPage() {
  ...
  const handleSubmit = (data: { name: string; slug: string; description?: string; address?: string; phone?: string }) => {
    if (editing) {
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">
          + Add Cafe
        </button>
      </div>
      ...
      <div className="space-y-2">
        {cafes.map((cafe) => (
          <div key={cafe.id} className="rounded-xl border bg-card p-4 flex items-center gap-3">
            ...
            <ActionMenu
              actions={[
                { label: "Edit", onClick: () => openEdit(cafe) },
                { label: "Delete", onClick: () => handleDelete(cafe.id), destructive: true },
              ]}
            />
          </div>
        ))}
